...
An AI-powered **Fake News Detection** web application with a stunning **Glassmorphism** UI, powered by the **Google Gemini API** and a secure **Python (Flask)** backend.
...
- 🤖 **Gemini AI integration** — Uses `gemini-2.5-flash` for fast, free fact-checking
...
Go to [Google AI Studio](https://aistudio.google.com/) → Create API key (free tier: 15 req/min).
...
3. Backend adds the **prompt** + API key, calls **Gemini API**
4. Gemini returns strict JSON: `{verdict, confidence_score, explanation}`
...
**Heroku / Render / Railway:** Set `GEMINI_API_KEY` as an environment variable in the platform's dashboard — no `.env` file needed.
